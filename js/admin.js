...
function renderOrders() {
    ...
    tbody.innerHTML = filtered.map(o => {
      ...
      <tr>
        ...
        <td>
          <ul class="dish-list">
            ${(o.dishes || []).map(d => `<li>${d}</li>`).join("")}
          </ul>
        </td>
        <td>
          <button onclick="deleteOrder()" class="delete-btn">🗑</button>
        </td>
      </tr>
    `;
    }).join("");
...
// ------------------------------
// DELETE (STATIC DEMO ONLY)
// ------------------------------
window.deleteOrder = function () {
    alert("Static demo only — cannot delete data.");
};
